 ...
 * jQuery plugin to help dealing with simplemodal dialogs
 * Copyright (c) 2010, Vladimir Dronnikov, dronnikov@gmail.com
 * Licensed under the MIT license
 * http://www.opensource.org/licenses/mit-license.php
 * Date: 2010-02-02
 *
 * Depends on: jquery.form, jquery.simplemodal
 *
 * Usage: jQuery.ajaxDialog(url, {options});
 *
 */
(function($) {
...
//	}
	...
	return this.each(function(){
		$(this).load(url, function(){
			...
			//console.info('loaded');
			//console.log(div);
			// N.B. file uploads can't be performed via XMLHttpRequest, so to reliably determine async request let's add _async=1 to the data
			$('form', div).append('<input type="hidden" name="_async" value="1" />');
			// asyncronous submit
			$('form', div).ajaxForm(function(text){
				//console.log(text);
				// empty response means OK, close the dialog
				if ('' === text) {
					$('a.cmd-cancel', div).click();
					// TODO: replace upper row with rendered content and remove editor row?
					try{reloadPage();}catch(x){alert('OK!');}
				// response is not empty -> reload dialog
				} else {
					div.html(text);
					//div.hide().html(text).show();
					//var d = $('<div></div>').hide().html(text);
					//div.html($('body', d).html());
					//d.remove();
				}
			});
			// reuse form buttons
			$('a.cmd-cancel', div).click(function(){
				/*if (options.onCancel)
					options.onCancel(div);
				if (options.onClose) {
					options.onClose(div);
				} else {
					$.modal.close();
			...
			$('a.cmd-apply', div).click(function(){
				if (options.onApply) {
					options.onApply(div);
				} else {
